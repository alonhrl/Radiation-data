...
def handle_month_end():
    ...
    fileName = 'bar_ilan-' + str(prevDate.month) + '-' + str(prevDate.year) + '.csv'
    ...
    column_names = [str(i) for i in range(startHour, startHour + (endHour - startHour + 1))]
...
with codecs.open('/Users/alonhrl/Documents/Alon/MA-MSC/development/Bar_Ilan/Solar_radiation_1h.csv', 'r', encoding='utf-8-sig') as csvfile:
    ...
    for l in reader:
        ...
        #ignore hours earlier than 06:00 and later than 18:00
        if  (int(currDate.hour) < 6) or (int(currDate.hour) > 17):
            continue
